...
delete from gboy_member where id!=1;
delete from gboy_money_finance where uid!=1;

delete from gboy_money_freed where uid!=1;

update gboy_money_finance set lock_wallet=0,futou_wallet=0,bocai_wallet=0,shop_wallet=0,usdt_wallet=0 ,grade=0,agency_level=0 where uid=1;
update gboy_money_freed SET total=0 where uid=1 ;


TRUNCATE table gboy_money_types;
TRUNCATE table gboy_member_chong;
TRUNCATE table gboy_member_log;
TRUNCATE table gboy_money_buy;
TRUNCATE table gboy_member_log;

select gboy_money_finance.lock_wallet,gboy_money_freed.total  from  gboy_money_finance  LEFT JOIN gboy_money_freed on gboy_money_finance.uid=gboy_money_freed.uid;