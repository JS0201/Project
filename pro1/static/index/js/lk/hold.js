...
function show_order_list() {
    ...
    $.each(resorderlist, function (k, v) {
        ...
        var timestamp = Date.parse(new Date());
        if (typeof (v.selltime) == "undefined") {
            v.selltime = 0
        }
        ...
        if (_end_time >= 0) {
            ...
            if (v.ostyle == 0) {
                ...
                if (chaprice > 0) {
                    ...
                    closeprice = v.fee;
                    closeprice_class = 'other_money';
                }
            } else {
                var ostyle_class = "buydown";
                var ostyle_name = " {:lang('Buy down')}";
            ...
            html += '<li ng-repeat="o" class="claerfix">\
                        <section>\
                            <p>\
                            ...
                            <p class="ng-binding">' + formatSeconds2(_end_time) + '</p>\
                        </section>\
                        <article class="">\
...
function setolist(types) {
    ...
    $.get(url, function (resdata) {
        ...
        $.each(res_list, function (k, v) {
            ...
            if (v.is_win == 1) {
                closeprice = (v.fee * (100 * 10 + v.endloss * 10) / 1000).toFixed(5);
                closeprice_class = 'in_money';
                closepriceUnit = '+'
            } else if (v.is_win == 2) {
                closeprice = v.fee*10000000/10000000;
                closeprice_class = 'out_money';
                closepriceUnit = '-'
            } else {
                closeprice = 0;
            ...
            html += '<li ng-repeat="o" class="clearfix" onclick="get_hold_order(' + v.oid + ')" >\
                        <section>\
                            <p>\
                                <span class="ng-binding">' + v.ptitle + '</span>\
                                <span  class="ng-binding ' + closeprice_class + '">\
                                ' + ostyle_name + '（' + v.fee*10000000/10000000 + '币）</span>\
                            </p>\
                            <p class="ng-binding">\
                            ...
                            <p class="ng-binding ' + closeprice_class + '">' + closepriceUnit + closeprice + '</p>\
...
function get_hold_order(oid) {
    ...
    $.get(url, function (data) {
        ...
        $('.order-modal-content .ploss').html(data.ploss > 0 ? '+'+ data.ploss : data.ploss );
        ...
        if (data.ploss < 0) {
            ...
            $('.order-modal-content .ploss').removeClass('fall');
            $('.order-modal-content .ploss').removeClass('rise');
            $('.order-modal-content .ploss').addClass('other_money');
        }
