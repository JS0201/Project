...
class TransactionController extends AdminController {
    ...
    public function buy(){
        ...
        if ($_GET['account_type']!='') {
            $sqlmap['account_type'] = $_GET['account_type'];
        }

        if ($_GET['keyword']) {
            $sqlmap['user_id|buy_user|seller_user|order_no'] = array('like', '%' . $_GET['keyword'] . '%');
        }
    ...
    public function cancel(){
        ...
        if($info=$t->where(array('id'=>$id))->find()){
            ...
            if($info['order_status']!=3 && $info['seller_id']){
                ...
                D('Member/MemberDetails')->add_details($info['seller_id'],$zh_types,$text,$info['num']+$info['fee']);
