...
function types_ofw($status=''){
    $status_array=array('1'=>'直推区块包奖励','2'=>'团队区块包奖励','3'=>'自购赠送区块包');
    if($status==''){
        return $status_array;
    }else{
        return $status_array[$status];
...
function article($cid){
    $category_id=$cid;
    $content=M('content');
    $cate=M('category')->where(array('id'=>$category_id))->find();//查找分类id是否存在
    if ($cate) {
        $gg_content=$content->where(array('category_id'=>$category_id,'display'=>'1'))->order('sort desc,id desc')->select();
        return $gg_content;
