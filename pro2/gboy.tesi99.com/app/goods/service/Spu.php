...
class Spu extends Model{
    ...
    {
       $this->model = model('goods/Spu');
        $this->goods_spec_model = model('goods/GoodsSpec');
       $this->sku_model = model('goods/Sku');
       $this->cat_model = model('goods/Category');
        $this->spec_model = model('goods/Spec');
       $this->index_model = model('goods/Index');
    ...
    public function get_list($sqlmap,$order='sort asc,id desc',$page=[]){
        ...
        if($lists) {
            foreach($lists as $k => $v) {
                $lists[$k]['sku_id'] = $this->sku_model->where("spu_id = {$v['id']}")->value('sku_id');
            }
        }
        return $lists;
    }
